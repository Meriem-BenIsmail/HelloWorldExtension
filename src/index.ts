...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (app: JupyterFrontEnd, palette: ICommandPalette) => {
    ...
    // Define a widget creator function-> then call it to make a new widget
    ...
    app.commands.addCommand(command, {
      ...
      execute: async () => {
        ...
        if ((await widget).isDisposed) {
          widget = newWidget();
        }
        if (!(await widget).isAttached) {
          // Attach the widget to the main work area if it's not there
          app.shell.add(await widget, 'main');
        }
        // Activate the widget
        app.shell.activateById((await widget).id);
      }
    });
